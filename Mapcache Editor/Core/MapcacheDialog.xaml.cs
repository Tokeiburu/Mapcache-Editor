...
using Mapcache_Editor.Core.Commands;
using TokeiLibrary;
...
namespace Mapcache_Editor.Core {
	...
	public partial class MapcacheDialog : TkWindow {
		...
		public MapcacheDialog()
			...
			}, new DefaultListViewComparer<MapInfo>(), new string[] { "Added", "Blue" });
			...
			try {
				...
				if (e.Data.GetDataPresent(DataFormats.FileDrop, true)) {
					...
					if (files.Length == 1 && files[0].IsExtension(".dat")) {
						_load(files[0]);
						_recentFilesManager.AddRecentFile(files[0]);
					}
					else {
